## This file is revised from SDSNE:https://github.com/kunzhan/SDSNE/tree/main
...
def load_data(dataFile, k, sigma): # {multi-view}
    ...
    for ii in range(0, len(Dist)):
        adj = bs_convert2sim_knn(Dist[ii], k, sigma)
        adj_dense = sparse_mx_to_torch_sparse_tensor(normalize_adj(adj)).to_dense()
        adjs_labels.append(adj_dense.bool() * 1.0 - torch.eye(len(adj_dense)))
        Sim.append(adj_dense)

    labels = torch.LongTensor(np.asarray(labels-1)).contiguous().view(-1)
    adjs = Sim
    num_graph = len(Sim)
    features = [torch.FloatTensor(f) for f in features]
    # feature_labels = [torch.FloatTensor(f).bool() * 1.0 for f in features]
